function CreateUser(container, settings, goBack) {
  ...
  this.createUserBox = function () {
    ...
    // Check the disableCookies flag before reading the cookie
    if (!goBack.disableCookies) {
      var nameCookie = document.cookie.split('; ').find(row => row.startsWith('username='));
      if (nameCookie) {
        var name = nameCookie.split('=')[1];
        var createNewU = new User(name, this.parentContainer, this.settings, goBack, this);
  ...
  this.return = function () {
    goBack.createGoBack(this, 'createUserBox', this.createUserDiv);
